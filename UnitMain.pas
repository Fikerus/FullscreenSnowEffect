...
begin
  Self.SetColor(RGB(255,0,255));
  SetWindowLongptr(Handle, GWL_EXSTYLE,
                  GetWindowLongptr(Handle, GWL_EXSTYLE) or WS_EX_LAYERED);

  SetLayeredWindowAttributes(Handle, RGB(255,0,255), // цвет прозрачности
                                         0,          // величина прозрачности
                       {LWA_ALPHA or} LWA_COLORKEY); // флаг для формы выключим
...
begin
  SetWindowPos(Handle, HWND_TOPMOST, 0, 0, 1900, 1080, $0000);
  // обновлаяем снег
  UpdateSnow;
